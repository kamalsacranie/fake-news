...
describe("convertTimestampToDate", () => {
  ...
  test("converts a created_at property to a date", () => {
    const timestamp = 1557572706232;
    const input = { created_at: timestamp };
    const result = convertTimestampToDate(input) as SeedComment;
  ...
  test("ignores includes any other key-value-pairs in returned object", () => {
    ...
    const result = convertTimestampToDate(input) as { [key: string]: any };
...
describe("createRef", () => {
  ...
  test("returns a reference object when passed an array with a single items", () => {
    ...
    let expected: { [key: string]: any } = { title1: 1 };
    expect(actual).toEqual(expected);
...
describe("formatComments", () => {
  ...
  // >>>>> This testing is no longer relevant as our seed data is not in this format
